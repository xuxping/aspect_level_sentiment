        ...
        with open(input_file, "r", encoding="utf-8") as f:
            ...
            for line in reader:
                if sys.version_info[0] == 2:
                    line = list(unicode(cell.replace(' ', ''), 'utf-8') for cell in line)
                lines.append(line)
            return lines


class SnProcessor(DataProcessor):
    ...
    def _load_data(self, data_path):
        ...
        for i in range(0, len(all_lines), 3):
            ...
            polarity = str(int(polarity) + 1)

            all_data.append((polarity, aspect, text))
        print(len(all_data))
        return all_data

    def get_train_examples(self, data_dir):
        """See base class."""
        data_path = './data/semeval14/Restaurants_Train.xml.seg'
        logger.info("LOOKING AT {}".format(data_path))
        return self._create_examples(self._load_data(data_path), 'train')
        # return self._create_examples(
        #     self._read_tsv(os.path.join(data_dir, "train.txt")), "train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        data_path = './data/semeval14/Restaurants_Test_Gold.xml.seg'
        # data_path = './data/semeval14/Restaurants_Train.xml.seg'

        return self._create_examples(self._load_data(data_path), 'dev')
...
def main():
    ...
    if args.do_train:
        model = BertForSequenceClassification.from_pretrained('./pretrain/',
                                                              cache_dir=cache_dir,
                                                              num_labels=num_labels)
    if args.do_eval:
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        # model = BertForSequenceClassification.from_pretrained('./snmodels/',
        #                                                       cache_dir=cache_dir,
        #                                                       num_labels=num_labels)
        pass
